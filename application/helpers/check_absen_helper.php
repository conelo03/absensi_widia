...
function check_jam($jam, $status, $raw = false, $divisi)
{
    $CI =& get_instance();
    $id_user = $CI->session->id_user;
    $user = $CI->db->get_where('users', ['id_user' => $id_user])->row_array();
    if ($jam) {
        $status = ucfirst($status);
        
        $CI->load->model('Jam_model', 'jam');
        $jam_kerja = $CI->jam->db->where('id_divisi', $divisi)->where('keterangan', $status)->get('jam')->row();
